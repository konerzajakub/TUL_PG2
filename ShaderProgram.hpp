...
public:
	...
	GLuint getID() const; // Get the shader program ID

    
private:
	GLuint ID{0}; // default = 0, empty shader

	std::string getShaderInfoLog(const GLuint obj);   // print compiler output  
	std::string getProgramInfoLog(const GLuint obj);  // print linker output
